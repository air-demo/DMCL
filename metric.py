...
def inference(loader, model, device, view, data_size, p_sample,adaptive_weight):
    ...
    for v in range(view):
        ...
        pred_vectors[v] = np.array(pred_vectors[v])
         #新增
    Hs = torch.cat(Hs, dim=0)  # 转换为张量
    Zs = torch.cat(Zs, dim=0)  # 转换为张量

    Hs = Hs.detach().cpu().numpy()  # 将 x_new 从 GPU 复制到 CPU，并转换为 NumPy 数组
    Zs = Zs.detach().cpu().numpy()  # 将 common_new 从 GPU 复制到 CPU，并转换为 NumPy 数组
    # 将 NumPy 数组保存为 .mat 文件
    data = {'x_new': Hs, 'common_new': Zs}
    savemat('Caltech-5V.mat', data)
...
def valid(model, device, dataset, view, data_size, class_num, p_sample,adaptive_weight):
    ...
    #获取真实标签和用于聚类的矩阵
    total_pred, pred_vectors, high_level_vectors, labels_vector, low_level_vectors  = inference(test_loader, model, device, view, data_size, p_sample,adaptive_weight)
    print('---------train over---------')
